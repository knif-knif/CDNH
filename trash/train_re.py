...
## Calculate JSD
def Calculate_JSD(tp):  
    JS_dist = Jensen_Shannon()
    JSD   = torch.zeros(args.num_train) 
    Psudo_Label = torch.zeros(args.num_train, args.num_classes)   
    ...
    for batch_idx, (img, label, index) in enumerate(eval_loader):
        ...
        with torch.no_grad():
            ...
            D = F.linear(code_T, PT if tp else PS) / 0.2
            Pseudo = F.linear(code_T, PT) / 0.2
            Pseudo = F.softmax(Pseudo.max() - Pseudo, -1)
        ...
        for b in range(img.size(0)):
            JSD[index[b]] = dist[b]
            Psudo_Label[index[b]] = Pseudo[b]

    return JSD if tp else 1 - JSD, Psudo_Label
...
def train(args, epoch, tp):
    ...
    for i, (img, label, _) in enumerate(conf_train):
        ...
        if not tp: label = 1 - label
        try:
            img_u, pseudo = next(unconf_iter)
        except:
            unconf_iter = iter(unconf_train)
            img_u, pseudo = next(unconf_iter)
        
        img_u, pseudo = img_u.cuda(), pseudo.cuda()
        optim.zero_grad()
        ...
        feat_u = Arch(imgT_u)
        code = Hash_T(feat) if tp else Hash_S(feat)
        code_u = Hash_T(feat_u) if tp else Hash_S(feat_u)

        l1 = HP_Loss(code, Hash_T.P if tp else Hash_S.P, label)
        l2 = HP_Loss(code_u, Hash_T.P if tp else Hash_S.P, pseudo) * args.lambda_d
        l3 = REG_Loss(code) * args.lambda_q
...
if __name__ == '__main__':
    ...
    for epoch in range(args.Epoch):
        ...
        if epoch < args.warm_up:
            ...
            prob_T, pseudo = Calculate_JSD(False)
            ...
            prob_S, pseudo = Calculate_JSD(True)
        ...
        scheduler.step()
    logger.debug(f"[MAX_mAP]: {MAX_mAP:.4f} ({MAX_EPOCH})")